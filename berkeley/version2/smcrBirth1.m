function [mov,aBirth,rBirth] = smcrBirth1(aBirth,rBirth,mov,movO,x,y,par,bFunction,s,t,birth,death);
...
MuProp = [mov.mu{s};  x(t+1,:) + sqrt(par.mu02)*randn(1,d)];
tmpDiag= par.P02*eye(mov.k(s)+d+1+1); 
tmpDiag(1:movO.k(s)+d+1,1:mov.k(s)+d+1) = mov.P{s};
PProp = tmpDiag;       
...
ratio = (likP*par.probDeath)/(likR*par.probUpdate);
